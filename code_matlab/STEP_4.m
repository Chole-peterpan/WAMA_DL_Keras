...
%% 参数设置
block_mat_path =       'H:\@data_NENs_recurrence\PNENs\data\flow1\2block';%block
augdict.mat_savepath = 'H:\@data_NENs_recurrence\PNENs\data\flow1\4aug';

%构建此循环的目的仅仅是为了利用matlab的代码折叠功能
%设置参数
for i = 1:1
    augdict.aug_num = 10000;%总扩增数量
    ...
    augdict.savefomat.param = [200,200,20];
...
for i = 1:length(subject)
    ...
    per_block_aug_num = ceil(per_sub_augnum/blocks);
...
for ii = 1:length(filename_list)
    ...
    for iii = 1:per_block_aug_num
        % 扩增
        if subject_ins.othermode
            [aug_data,aug_data_othermode,aug_detail] = aug43D(data,  augdict,  subject_ins.othermode,  data_othermode);
        else
            [aug_data,~                 ,aug_detail] = aug43D(data,  augdict,  subject_ins.othermode,  []);
        end
        ...
        h5create(finalpath, '/label_1', size(subject_ins.label_1),'Datatype','single');
        h5write(finalpath, '/label_1', subject_ins.label_1);
        h5create(finalpath, '/label_2', size(subject_ins.label_2),'Datatype','single');
        h5write(finalpath, '/label_2', subject_ins.label_2);
        h5create(finalpath, '/label_3', size(subject_ins.label_3),'Datatype','single');
        h5write(finalpath, '/label_3', subject_ins.label_3);
        h5create(finalpath, '/label', size(subject_ins.label),'Datatype','single');
        h5write(finalpath, '/label', subject_ins.label);
        h5create(finalpath, '/label1', size(subject_ins.label1),'Datatype','single');
        h5write(finalpath, '/label1', subject_ins.label1);
        h5create(finalpath, '/label2', size(subject_ins.label2),'Datatype','single');
        h5write(finalpath, '/label2', subject_ins.label2);
        h5create(finalpath, '/label3', size(subject_ins.label3),'Datatype','single');
        h5write(finalpath, '/label3', subject_ins.label3);
