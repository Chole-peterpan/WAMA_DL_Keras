...
def _grouped_convolution_block_3D(input, grouped_channels, cardinality, strides,gpname):
    ...
    if cardinality == 1:
        ...
        x = Conv3D(grouped_channels, (3, 3, 3), padding='same', use_bias=False, strides=strides, kernel_initializer='he_normal',name=gpname+'_c')(init)
     #   x = BatchNormalization(axis=channel_axis,name = gpname+'_bn')(x)
     #   x = Activation('relu')(x)
        return x
    ...
    for c in range(cardinality):
        ...
        x = Conv3D(grouped_channels, (3, 3, 3), padding='same', use_bias=False, strides=strides, kernel_initializer='he_normal',name = gpname+'_c'+str(c))(x)
        group_list.append(x)
...
def conv_block_or(x, nb_filters, name, cardinality=32):
    ...
    grouped_channels = k2 // cardinality  # 除法运算并向下取整
    out = _grouped_convolution_block_3D(out, grouped_channels, cardinality, strides=2,gpname=convname2)
    #out = Conv3D(k2, kernel_size, strides=1, padding='same', kernel_initializer='he_normal', name = convname2)(out)
    out = BatchNormalization(axis=-1, epsilon=1e-6, name=bnname2)(out)
    out = Activation('relu')(out)
