...
from keras.layers import Input, merge,Conv3D,BatchNormalization,Conv3DTranspose, UpSampling3D, MaxPooling3D, AveragePooling3D, \
    GlobalAveragePooling3D,Dense,Flatten,Lambda,Dropout,Activation,ZeroPadding3D,Convolution3D,Reshape,multiply
...
def SparseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
              bottleneck=False, reduction=0.0, dropout_rate=0.0, subsample_initial_block=False,
    ...
    x = _create_dense_net(classes, img_input, include_top, depth, nb_dense_block,
                          ...
                          dropout_rate, subsample_initial_block, activation)
    ...
    # model.compile(optimizer=SGD(lr = 1e-4, momentum = 0.9), loss = 'binary_crossentropy', metrics= [y_t, y_pre, Acc] )
    # model.compile(optimizer=Adam(lr=1e-3), loss='binary_crossentropy', metrics=[y_t, y_pre, Acc])
...
def _create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
    ...
    for block_idx in range(nb_dense_block - 1):
        ...
        x = _transition_block(x, nb_filter, compression=compression)
        nb_filter = int(nb_filter * compression)

        # # # Add SE blocks
        # x = squeeze_excite_block3d(x, block_idx)
...
def squeeze_excite_block3d(input,stage, ratio = 2):
    nb_channel = int_shape(input)[-1]
    SE_shape = (1, 1, 1, nb_channel)

    out = GlobalAveragePooling3D(name='SE_'+str(stage)+'_GlobalPooling')(input)
    out = Reshape(SE_shape)(out)
    out = Dense(nb_channel//ratio, activation = 'relu', kernel_initializer = 'he_normal', use_bias = False,name='SE_'+str(stage)+'_FC_ReLU')(out)  # //表示相除并取整
    out = Dense(nb_channel, activation='sigmoid', kernel_initializer='he_normal', use_bias=False,name='SE_'+str(stage)+'_FC_sigmoid')(out)
