...
#现在只用这个(针对双输入的)
#v2.0版本
def test_on_model4_subject_4_miulti(model, test_list, iters,
                           data_input_shape, label_shape,
                           id_savepath, label_savepath, pre_savepath, othermode_path,
                           label_index = 'label_2'):
    """
    ...
    # 构建容器
    data_input_1 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container
    data_input_2 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container 4 anothor mode
    label_input_1 = np.zeros([1] + label_shape)
    ...
    for read_num in Num_list_test:
        ...
        batch_x_v = H5_file['data'][:]
        batch_y1 = H5_file[label_index][:]
        print(batch_y1)
        batch_x_t_v = np.transpose(batch_x_v, (1, 2, 0))
        data_input_2[0, :, :, :, 0] = batch_x_t_v[:, :, 0:16]
        H5_file.close()
        ...
        result_loss = model.test_on_batch([data_input_1,data_input_2], label_input_1)
...
# 多气象使用的
def test_on_model4_subject4_or_train_4multi(model, test_list, data_input_shape, label_shape,
                                            othermode_path, label_index='label_2'):
    # 保存预测值,同时保存最终指标
    # 精确到样本,指标计算单位为样本
    data_input_1 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container
    data_input_2 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container 4 another mode (v)
    label_input_1 = np.zeros([1] + label_shape)
