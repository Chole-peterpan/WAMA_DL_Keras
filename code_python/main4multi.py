...
from w_resnet import resnet_nobn, se_resnet, resnet_or,multiinput_resnet
...
aug_path_2 = param_dict['aug_subject_path_othermode']
or_path_2 = param_dict['or_subject_path_othermode']
...
if __name__ == "__main__":
    ...
    data_input_2 = np.zeros([batch_size] + data_input_shape + [1], dtype=np.float32)  # net input container 4 another input
    label_input_c = np.zeros([batch_size] + label_shape)
    # 构建网络并compile
    d_model = multiinput_resnet(classes=2)
    d_model.compile(optimizer=adam(lr=init_lr), loss=EuiLoss_new, metrics=[y_t, y_pre, Acc])
    # pause()  # identify
    ...
    for i in range(max_iter):
        ...
        for ii in range(batch_size):
            ...
            # read data from h5 based on a : 4 v ---------------------------------
            read_name_v = name2othermode(aug_path_2, read_name, 300)
        ...
        # train on model
        # cost = d_model.train_on_batch(data_input_1, label_input_c)
        cost = d_model.train_on_batch([data_input_1, data_input_2], label_input_c)
        # pre = d_model.predict_on_batch(data_input_1)
        ...
        if Iter >= min_verify_Iters and Iter % verify_Iters_step == 0:
            # ver
            vre_result = test_on_model4_subject_4_miulti(model=d_model,
            ...
            or_train_result = test_on_model4_subject4_or_train_4multi(model=d_model,
        ...
        if Iter == optimizer_switch_point:
            ...
            d_model.compile(optimizer=SGD(lr=lr_new, momentum=0.9), loss=EuiLoss_new, metrics=[y_t, y_pre, Acc])
