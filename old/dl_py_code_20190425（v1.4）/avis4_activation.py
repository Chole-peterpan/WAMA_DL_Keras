...
model.load_weights(filepath='/data/XS_Aug_model_result/model_templete/qianyi/model_qianyi_new/2LEVEL/m_70000_model.h5',by_name=True)
...
layer_idx_conv = utils.find_layer_idx(model, 'L2_block1conv1')
...
max_activition_norm = visualize_activation(model, layer_idx_conv, filter_indices=1, max_iter=5000, input_modifiers=[Jitter(16)],  verbose=True)
max_activition_norm = np.squeeze(max_activition_norm, axis=-1)
for ii in range(16):
    plt.figure()
    max_activition_piece = max_activition_norm[:, :, ii]
