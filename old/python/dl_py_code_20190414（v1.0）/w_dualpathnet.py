...
# ----------------------------------------------------------------------------------
def _initial_conv_block_inception(input, initial_conv_filters):
    ...
    x = Conv3D(initial_conv_filters, (7, 7, 5), strides=(2, 2, 1), padding='same', use_bias=False, kernel_initializer='he_normal', name='init_conv1')(input)
...
# ----------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------
def _bn_relu_conv_block(input, filters, kernel=(3, 3, 3), stride=(1, 1, 1)):
    ...
    x = Conv3D(filters, kernel, padding='same', use_bias=False, kernel_initializer='he_normal', strides=stride)(input)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
    return x
# ----------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------
def _dual_path_block(input, pointwise_filters_a, grouped_conv_filters_b, pointwise_filters_c, filter_increment, cardinality, block_type='normal'):
    '''
    ...
    if projection:
        # 先过一个卷积层
        projection_path = _bn_relu_conv_block(init, filters=pointwise_filters_c + 2 * filter_increment, kernel=(1, 1, 1), stride=stride)
    ...
    x = _bn_relu_conv_block(init, filters=pointwise_filters_a, kernel=(1, 1, 1))
    # 可以看出，grouped_channels*cardinality = pointwise_filters_a，也就是pointwise_filters_a = grouped_conv_filters_b
    x = _grouped_convolution_block_3D(x, grouped_channels=grouped_channels, cardinality=cardinality, strides=stride)
    x = _bn_relu_conv_block(x, filters=pointwise_filters_c + filter_increment, kernel=(1, 1, 1))
...
# ----------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------
#def dual_path_net(initial_conv_filters = 64, filter_increment = [16, 32, 24, 128], depth = [3, 4, 20, 3], cardinality=32, width=3, pooling='max-avg'):
def dual_path_net(initial_conv_filters, filter_increment, depth, cardinality, width, pooling='max-avg'):
    '''
    Args:
    ...
    x = _dual_path_block(x, pointwise_filters_a=filters, grouped_conv_filters_b=filters, pointwise_filters_c=base_filters, filter_increment=filter_inc, cardinality=cardinality, block_type='projection')  #

    for i in range(N[0] - 1):
        x = _dual_path_block(x, pointwise_filters_a=filters, grouped_conv_filters_b=filters, pointwise_filters_c=base_filters, filter_increment=filter_inc, cardinality=cardinality, block_type='normal')  #
    ...
    for k in range(1, len(N)):
        ...
        x = _dual_path_block(x, pointwise_filters_a=filters, grouped_conv_filters_b=filters, pointwise_filters_c=base_filters, filter_increment=filter_inc, cardinality=cardinality, block_type='downsample')  #
