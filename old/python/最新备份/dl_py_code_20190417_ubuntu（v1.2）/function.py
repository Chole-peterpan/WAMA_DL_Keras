...
#现在只用这个
def test_on_model4_subject(model, test_list, iters, save_path, data_input_shape, label_shape, front_name, file_sep):
    # 保存预测值,同时保存最终指标
    # 精确到样本,指标计算单位为样本
    data_input_1 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container
...
def test_on_model4_subject4_or_train(model, test_list, data_input_shape, label_shape):
    # 保存预测值,同时保存最终指标
    # 精确到样本,指标计算单位为样本
    data_input_1 = np.zeros([1] + data_input_shape + [1], dtype=np.float32)  # net input container
    label_input_1 = np.zeros([1] + label_shape)
