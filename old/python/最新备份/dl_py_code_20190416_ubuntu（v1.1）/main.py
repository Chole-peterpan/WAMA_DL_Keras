...
# 分折信息导入
aug_path = r'H:\@data_liaoxiao\3aug_2500'  # CD patient is positive
test_path = r'H:\@data_liaoxiao\4test'
folder_path = r'H:\@data_liaoxiao\@folder_5_2500'# 分折信息文件路径

Result_save_Path = r'/data/XS_Aug_model_result/model_templete/diploma_real/test8'#============================#!!!!!!
#model_save_Path = r'/data/XS_Aug_model_result/model_templete/diploma/pnens_zhuanyi_densenet/fold2'#============================暂时用不到保存模型
foldname = '3'# 只改动这里即可,无需设定分折文件#============================
task_name = 'test8'#!!!!!!
#======================================================================================================================
if os_stage == "W":
    file_sep = r"\\"
...
if __name__ == "__main__":
    ...
    # txt as a log
    minibatch_loss_txt = Result_save_Path + file_sep[0]+'@' + foldname + '_loss.txt'  # minibatch上的loss
    verify_result_txt = Result_save_Path +file_sep[0]+ '@' + foldname + '_ver_result.txt'  # 验证集的所有指标
    test_result_txt = Result_save_Path +file_sep[0]+ '@' + foldname + '_test_result.txt'  # 测试集的所有指标

    verify_acc_txt = Result_save_Path + file_sep[0]+'@' + foldname + '_ver_acc.txt'  # 验证集的acc
    verify_AUC_txt = Result_save_Path + file_sep[0]+'@' + foldname + '_ver_AUC.txt'  # 验证集的AUC
    test_acc_txt = Result_save_Path +file_sep[0]+ '@' + foldname + '_test_acc.txt'  # 测试集的acc
    test_AUC_txt = Result_save_Path + file_sep[0]+'@' + foldname + '_test_AUC.txt'  # 测试集的AUC
