...
# 分折信息导入
aug_path = r'H:\@data_liaoxiao\3aug_10000'  # CD patient is positive
test_path = r'H:\@data_liaoxiao\4test'
folder_path = r'H:\@data_liaoxiao\@folder_5_10000new'# 分折信息文件路径

Result_save_Path = r'G:\qweqweqweqwe'#============================#!!!!!! 保存路径保存路径保存路径保存路径保存路径
...
if __name__ == "__main__":
    ...
    # ???????
    d_model = vgg16_w_3d_gb(classes=1)
    d_model.compile(optimizer=SGD(lr=1e-6), loss=EuiLoss, metrics=[y_t, y_pre, Acc])
    #===================================================================================
    ...
    for i in range(max_iter):
        ...
        if Iter >= min_verify_Iters and Iter % verify_Iters_step == 0:
            ...
            # func 4 test every 'verify_Iters_step reached' time ====================================这个需要换一个函数，因为test_on_model4_subject这个函数会保存预测结果，但是训练集没必要保存预测结果
