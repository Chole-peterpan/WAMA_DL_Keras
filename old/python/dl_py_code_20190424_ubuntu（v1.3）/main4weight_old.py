...
from function import pause, test_on_model4_subject4grt_weight, test_on_model4_subject4_or_train4_getweight
...
batch_size = 5
max_iter = 100000
min_verify_Iters = 10000  # 开始验证的最小迭代次数:batch*diedaishu = epoch
verify_Iters_step = 100 # verify every 'verify_Iters_step' times
data_input_shape = [280, 280, 16]
label_shape = [2]
min_weightsaveTters = 10000   # 开始保存模型的最小迭代次数:batch*diedaishu = epoch
min_weightsave_step = 5000  # 保存模型的步长

# label的第一个维度是sample（可以理解为行数）
# step4: parameter set finished

# 分折信息导入
aug_path = r'/data/@data_pnens_zhuanyi_dl/data_aug/a'  # CD patient is positive
test_path = r'/data/@data_pnens_zhuanyi_dl/data_test/a'
folder_path = r'/data/@data_pnens_zhuanyi_dl/CVfold5_4qianyi'# 分折信息文件路径

Result_save_Path = r'/data/XS_Aug_model_result/model_templete/qianyi_result'#============================#!!!!!! 保存路径保存路径保存路径保存路径保存路径
model_save_Path = r'/data/XS_Aug_model_result/model_templete/qianyi_model'#============================保存模型
foldname = '1'# 只改动这里即可,无需设定分折文件#============================
task_name = 'qianyiweight'#!!!!!!
...
if __name__ == "__main__":
    ...
    for i in range(max_iter):
        ...
        if Iter >= min_verify_Iters and Iter % verify_Iters_step == 0:
            ...
            vre_result = test_on_model4_subject4grt_weight(model=d_model, test_list=H5_List_verify, iters=Iter, save_path=Result_save_Path, data_input_shape=data_input_shape, label_shape=label_shape, front_name='vre', file_sep=file_sep,label_index=label_index)
            # save ver_result
            ...
            test_result = test_on_model4_subject4grt_weight(model=d_model, test_list=H5_List_test, iters=Iter, save_path=Result_save_Path, data_input_shape=data_input_shape, label_shape=label_shape, front_name='test', file_sep=file_sep,label_index=label_index)
            ...
            or_train_result = test_on_model4_subject4_or_train4_getweight(model=d_model, test_list=H5_List_or_train, data_input_shape=data_input_shape, label_shape=label_shape,label_index=label_index)
        ...
        if Iter >= min_weightsaveTters and Iter % min_weightsave_step == 0:
            d_model.save(model_save_Path + '/m_' + str(Iter) + '_model.h5')
