...
batch_size = 6
max_iter = 200000
min_verify_Iters = 5000   # 开始验证的最小迭代次数:batch*diedaishu = epoch
verify_Iters_step = 50  # verify every 'verify_Iters_step' times
data_input_shape = [280, 280, 16]
label_index = 'label3'#label3
label_shape = [2]
model_save_step = 5000 #迁移权重的迭代数步伐长度
# label的第一个维度是sample（可以理解为行数）
# step4: parameter set finished

# 分折信息导入
aug_path = r'/data/@data_NENs_recurrent/3aug_35000'  # CD patient is positive
test_path = r'/data/@data_NENs_recurrent/4test'
folder_path = r'/data/@data_NENs_recurrent/CVfolder5'# 分折信息文件路径

Result_save_Path = r'/data/XS_Aug_model_result/model_templete/qianyi/qianyi_real_new/3RECURRENT'#============================#!!!!!! 保存路径保存路径保存路径保存路径保存路径
model_save_Path = r'/data/XS_Aug_model_result/model_templete/qianyi/model_qianyi_new/3RECURRENT'#============================暂时用不到保存模型
foldname = '1'# 只改动这里即可,无需设定分折文件#============================
task_name = 'RECURRENT_FOLD1_qianyi'#!!!!!!
...
if __name__ == "__main__":
    ...
    d_model.load_weights(r'/data/XS_Aug_model_result/model_templete/qianyi/model_qianyi_new/3RECURRENT/m_20000_model.h5')
