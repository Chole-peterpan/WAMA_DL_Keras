...
iiindex = 20000;%按照迭代数卡阈值，防止早期结果参与
%% 显示一下minibatch_loss
...
ver_result = ver_result(ver_result(:,1)>iiindex,:);%首先把小于一定迭代数的卡掉
...
%% 将所有test迭代数对应的预测值全部读进来，label也读进来
b_test_label_new = [];
b_test_pre_value_new = [];
b_pp_id = [];
for i = 1:size(final_test_result)
    orlabel_path = strcat(fold_path,filesep,'testorginal_',num2str(final_test_result(i,1)),'.txt');
    preval_path = strcat(fold_path,filesep,'testpredict_',num2str(final_test_result(i,1)),'.txt');
    ID_path = strcat(fold_path,filesep,'testsubjectid_',num2str(final_test_result(i,1)),'.txt');
...
%% 按列取均值（相当于把很多个迭代数汇总起来,迭代数的数量其实就是前面的min_test_select_num）
test_ID = b_pp_id(1,:);
...
%% save
save(strcat(save_path,filesep,'folder_',fold_name),'test_label','test_prevalue','test_ID'); 
