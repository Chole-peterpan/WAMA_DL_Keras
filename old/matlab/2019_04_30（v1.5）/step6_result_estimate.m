...
fold_name = '2';
a = 2;
indexx = 7;%使用指标的序号，1就是第1指标,最终会按照这个指标来选择权重，指标越大表示模型越好（除了loss越小表示模型越好）
batch_size = 6;
...
or_trainloss = smooth(ortrain_loss,50);
...
test_orlabel_path = strcat(fold_path,filesep,tmp_str_s,'orginal_',num2str(final_value(1)),'.txt');
test_prevalue_path = strcat(fold_path,filesep,tmp_str_s,'predict_',num2str(final_value(1)),'.txt');
test_subID_path = strcat(fold_path,filesep,tmp_str_s,'subjectid_',num2str(final_value(1)),'.txt');
