...
savepath = 'H:\@data_NENs_recurrent\CVfolder5';
%% AUG data 4 train and verify（h5文件）
% AUG ：all negative object fold in a cell
AUG_fold_cell{1}='H:\@data_NENs_recurrent\3aug_35000';
...
test_fold_cell{1}='G:\@data_NENs_recurrence\PNENs\data\a\4test';
...
n_fold_cell{1}='G:\@data_NENs_recurrence\PNENs\data\a\1mat\a';
% n_fold_cell{2}='';
% n_fold_cell{3}='';
% n_fold_cell{4}='';

% cut ：all positive object fold in a cell
p_fold_cell{1}='G:\@data_NENs_recurrence\PNENs\data\a\1mat\b';
% p_fold_cell{2}='';
% p_fold_cell{3}='';
% p_fold_cell{4}='';
...
for iiii = 1:K_fold
    ...
    % ==========================================================
    % 从block文件（test文件，即未扩增的文件）中挑选对应折的test数据
    final_test = {};
    ...
    % ==========================================================
    %从block文件（test文件，即未扩增的文件）中挑选对应折的verify数据
    final_verify = {};
    for iii = 1:size(test_filename_final,1)
        tmp_str = test_filename_final{iii};
        tmp_index = find(tmp_str == '_');
    ...
    % ==========================================================
    %从aug文件中挑选对应折的train数据（扩增的数据集，用来给网络minibatch训练）
    final_train = {};
    ...
    for iii = 1:size(test_filename_final,1)
        ...
        tmp_str_p = tmp_str(1:tmp_index(1)-1);
        if  ismember(tmp_str_p,folder(iiii).train)
            final_or_train = [final_or_train;tmp_str];
        end
    end
    folder_final(iiii).or_train = final_or_train;
...
for iii = 1:K_fold
    ...
    savename4or_train = strcat(savepath,filesep,'fold_',num2str(iii),'_ortrain.txt');
    ...
    fclose(fid);
    disp('finish aug train filepath trans'); 
    
    % save the or train in
    % TXT(没扩增的训练集，单纯用于和测试集验证集一起predict的)===============
    fid=fopen(savename4or_train, 'w'); % 文件c为数据整合后的文件
    ortrain = folder_final(iii).or_train;
    for n=1:length(folder_final(iii).or_train)
        fprintf(fid,'%s\n',char(ortrain{n}));    %
    end
    fclose(fid);
    disp('finish or train filepath trans'); 
end
